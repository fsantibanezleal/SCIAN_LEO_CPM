...
GL_Camera::GL_Camera(float positionX, float positionY, float positionZ)
{
	m_show_world=false;
	m_camara_move=false;
	m_haptic_set=false;
...
void GL_Camera::SetCamera( float viewX, float viewY, float viewZ,
...
void GL_Camera::SetParametrosHapticos(point_3D _h_Eye,point_3D _h_Center,point_3D _h_Up)
{
	if(!m_haptic_set)
		return;
	camView = _h_Eye;
...
void GL_Camera::Examinar(int x, int y)
{
...
void GL_Camera::MouseMotion(int x, int y)
{
	old_y = y;
	old_x = x;
}
void GL_Camera::SetDependentParametersCamera(void)
{
...
void GL_Camera::SetGLCamera( void )
{
	float	 fovy;
...
void GL_Camera::GetMatrix(GLdouble *m, GLdouble *p, GLint *v)
{
	for (int i=0;i<16;i++)
	{
		p[i] =	projection[i]; 
...
void GL_Camera::GetInvMatrix(float* m)
{
	for (int i=0;i<16;i++)
		m[i] =	m_proyection_invertida[i]; 
}

void GL_Camera::GetDirMatrix(float* m)
{
	for (int i=0;i<16;i++)
		m[i] =	look_matrix[i]; 
}

void GL_Camera::SetGLAspectRatioCamera( void )
{
...
void GL_Camera::YawCamera(float angle )
{
	float vIn[3];
...
void GL_Camera::Rotar_Latitud(float inc )
{
	float vIn[3];
...
void GL_Camera::Rotar_Longitud(float inc )
{
	float vIn[3];	
...
void GL_Camera::Rotar(float inc_x, float inc_y)
{
	float vIn[3];	
...
void GL_Camera::SetOldXY(int x, int y)
{
	old_x=x;
	old_y=y;
}
void GL_Camera::Andar(int x, int y)
{
...
void GL_Camera::Zoom(int x, int y)
{
	float zoom;
...
void GL_Camera::ZoomHaptic(int tipo)
{
	float zoom;
	switch(tipo)
	{
...
void GL_Camera::ClickMouse(int x, int y)
{
	SetOldXY(x,y);
...
void GL_Camera::KeyBoard(int key)
{
	if(key==KEY_T_F1)
	{
		m_MouseMotion=true;
...
void GL_Camera::MouseMove(int x, int y)
{
	if( m_MouseMotion) 
	{
		MouseMotion(x,y);
...
//=============================================================================
//=============================================================================
void GL_Camera::VectorRotY( float *vIn, float inc )
{
	float	 alpha;
	float	 modZX;
	float	 mod;
...
void GL_Camera::VectorRotXZ( float *vIn, float inc, int flagStop )
{
	float	 alpha, beta;
	float	 mod;
...
void GL_Camera::VectorRot( float *vIn, float inc_x, float inc_y)
{
	float	 alpha, beta;
	float	 mod;
...
bool GL_Camera::InversaLU(float *m_origen)
{
	int filas,columnas;
	filas=columnas=4;
...
GLboolean GL_Camera::invert(GLdouble src[16], GLdouble inverse[16])
{
    double t;
    int i, j, k, swap;
    GLdouble tmp[4][4];
...
void GL_Camera::identity(GLdouble m[16])
{
...
void GL_Camera::MultMxV(float *_C, float *A, float *_B)
{
	int i,k;
	float sum = 0.0;
	float C[4]={0,0,0,1};
